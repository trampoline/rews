...
REWS is a Ruby client for Exchange Web Services, currently focussed on retrieving email messages
...
to use : 
  ...
  mids = inbox.find_message_ids(:restriction=>[:<=, "item:DateTimeReceived",DateTime.now], 
                                :sort_order=>[["item:DateTimeReceived", "Ascending"]],
                                :indexed_page_item_view=>{:max_entries_returned=>10, :offset=>10})

  # get properties for a bunch of messages in one hit
  messages = inbox.get_messages(mids, :item_shape=>{
                                         :base_shape=>:IdOnly,
